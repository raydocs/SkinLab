...
struct ForecastEngine {
    ...
    func forecast(
        ...
        for i in 1 ... horizon {
            ...
            let intervalWidth = tValue * standardError *
                sqrt(1 + 1.0 / Double(values.count) + pow(distanceFromMean, 2) /
                    sumSquaredDeviations(days.map { Double($0) }))
    ...
    func predictAcneTrend(
        ...
        let riskLevel: RiskLevel = if predictedValue >= 7 && recentTrend > 0.3 {
            .high
    ...
    func predictSensitivityTrend(
        ...
        let days = dates.enumerated().map(\.offset)
    ...
    private func getSeasonalAdjustment(for season: Season) -> Double {
        switch season {
        case .spring: 0.5 // 春季敏感度略高
        case .summer: -0.3 // 夏季可能略低
        case .autumn: 0.2 // 秋季轻微增加
        case .winter: 0.8 // 冬季敏感度较高
        }
    }
