...
struct ReliabilityScorer {
    ...
    func score(
        ...
        let expectedDate = Calendar.current.date(byAdding: .day, value: expectedDay, to: session.startDate) ?? checkIn
            .captureDate
        let daysOffTarget = Calendar.current.dateComponents([.day], from: expectedDate, to: checkIn.captureDate)
            .day ?? 0
        ...
        let level: ReliabilityMetadata.ReliabilityLevel = if score >= 0.7 {
            .high
        } else if score >= 0.4 {
            .medium
        } else {
    ...
    func scoreAll(
        ...
        let positionCounts = Dictionary(
            grouping: checkIns.compactMap { $0.photoStandardization?.cameraPosition },
            by: { $0 }
        )
        .mapValues { $0.count }
        ...
        for checkIn in checkIns {
            ...
            let positionConsistent: Bool = if let commonPosition,
                                              let checkInPosition = checkIn.photoStandardization?.cameraPosition {
                checkInPosition == commonPosition
            } else {
                true
            }
...
extension TrackingSession {
    func expectedDay(for day: Int) -> Int {
        TrackingConstants.checkInDays.min(by: { abs($0 - day) < abs($1 - day) }) ?? day
    }
}
